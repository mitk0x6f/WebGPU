...
) -> VertexOut
{
    ...
    // Transform position to world space (skybox positioned at camera)
    let worldPos = (uModel * vec4<f32>(position, 1.0)).xyz;

    // Use world position as cubemap direction
    // Since skybox is centered at camera, worldPos is the direction from camera
